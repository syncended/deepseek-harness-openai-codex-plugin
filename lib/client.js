...
.dshCodexCode{position:relative;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);border-radius:8px;padding:12px 92px;font:600 22px/28px ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.12em;text-align:center;cursor:pointer;transition:border-color .15s ease,background-color .15s ease}
.dshCodexCode:hover{border-color:var(--dsw-alias-state-business-primary)}
.dshCodexCode[data-copied=true]{border-color:var(--dsw-alias-state-business-primary);background:var(--dsw-alias-interactive-bg-hover)}
.dshCodexCodeValue{display:block;overflow:hidden;text-overflow:ellipsis}
.dshCodexCopyFeedback{position:absolute;right:14px;top:50%;transform:translateY(-50%);display:flex;align-items:center;gap:5px;color:var(--dsw-alias-label-tertiary);font:500 12px/18px system-ui,sans-serif;letter-spacing:0;white-space:nowrap}
.dshCodexCode[data-copied=true] .dshCodexCopyFeedback{color:var(--dsw-alias-state-business-primary)}
.dshCodexSrOnly{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}
@media(max-width:420px){.dshCodexCode{padding-right:48px;padding-left:48px}.dshCodexCopyFeedback{right:10px}.dshCodexCopyFeedbackText{display:none}}
...
.dshCodexLimit{display:flex;flex-direction:column;gap:7px}.dshCodexLimitHead{display:flex;justify-content:space-between;font-size:12px}.dshCodexLimitName{color:var(--dsw-alias-label-secondary)}.dshCodexLimitValue{font-weight:600}.dshCodexLimitTrack{height:7px;border-radius:999px;background:var(--dsw-alias-bg-layer-3);overflow:hidden}.dshCodexLimitBar{height:100%;border-radius:inherit;background:var(--dsw-alias-state-business-primary)}.dshCodexLimitReset,.dshCodexLimitsMeta{color:var(--dsw-alias-label-tertiary);font-size:11px}.dshCodexLimitsSectionTitle{border-top:1px solid var(--dsw-alias-border-l2);padding-top:12px;margin-top:2px;color:var(--dsw-alias-label-tertiary);font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.04em}.dshCodexLimitsMeta{border-top:1px solid var(--dsw-alias-border-l2);padding-top:10px;display:flex;gap:10px;flex-wrap:wrap}
`;
    ...
    const dictionaries = {
      zh: {
        ...
        "limits.primary": "5 小时限额", "limits.secondary": "7 天限额", "limits.named": "{name} · {window}", "limits.modelSpecific": "模型专属限额", "limits.codeReview": "代码审查限额",
      ...
      en: {
        ...
        "limits.primary": "5-hour limit", "limits.secondary": "7-day limit", "limits.named": "{name} · {window}", "limits.modelSpecific": "Model-specific limits", "limits.codeReview": "Code review limit",
    ...
    function windowLabel(value, fallback, t) {
      if (value?.windowSeconds === 5 * 60 * 60) return t("limits.primary");
      if (value?.windowSeconds === 7 * 24 * 60 * 60) return t("limits.secondary");
      return fallback;
    }

    function usageGroups(value, t) {
      if (!value) return { base: [], additional: [] };
      const byDuration = (left, right) => (left.value.windowSeconds ?? Infinity) - (right.value.windowSeconds ?? Infinity);
      const base = [
        { key: "primary", value: value.primary, fallback: t("limits.primary") },
        { key: "secondary", value: value.secondary, fallback: t("limits.secondary") },
      ]
        .filter((row) => row.value)
        .map((row) => ({ ...row, label: windowLabel(row.value, row.fallback, t) }))
        .sort(byDuration);
      const additional = (value.additional ?? [])
        .map((item, index) => {
          const duration = windowLabel(item, null, t);
          return {
            key: `additional-${item.name}-${item.window ?? index}`,
            value: item,
            label: duration ? format(t, "limits.named", { name: item.name, window: duration }) : item.name,
          };
        })
        .sort(byDuration);
    ...
    function CodexLimits({ wide, t }) {
      ...
      const groups = usageGroups(value, t);
      const rows = [...groups.base, ...groups.additional];
      const summary = rows.length ? Math.round(Math.min(...rows.map((row) => remaining(row.value)))) : undefined;
      const meta = [];
        ...
        open
          ? React.createElement(
              ...
              React.createElement(
                ...
                groups.base.map((row) => React.createElement(LimitRow, { key: row.key, label: row.label, value: row.value, t })),
                value ? React.createElement(LimitRow, { label: t("limits.codeReview"), value: value.codeReview, t }) : null,
                groups.additional.length
                  ? React.createElement("div", { className: "dshCodexLimitsSectionTitle" }, t("limits.modelSpecific"))
                  : null,
                groups.additional.map((row) => React.createElement(LimitRow, { key: row.key, label: row.label, value: row.value, t })),
    ...
    function CodexSettings({ t }) {
      ...
      const userCode = pending ? login.userCode : undefined;
      ...
      const copyCode = async () => {
        if (!userCode || copied) return;
        if (!(await writeClipboard(userCode))) return;
        setCopied(true);
            ...
            ? React.createElement(
                ...
                React.createElement(
                  "button",
                  {
                    ...
                    "aria-label": `${copied ? t("copied") : t("copy")}: ${userCode}`,
                    onClick: () => void copyCode(),
                  },
                  React.createElement("span", { className: "dshCodexCodeValue" }, userCode),
                  React.createElement(
                    "span",
                    { className: "dshCodexCopyFeedback", "aria-hidden": true },
                    copied
                      ? React.createElement(IconCheckOutline16, { size: 14 })
                      : React.createElement(IconCopyOutline16, { size: 14 }),
                    copied ? React.createElement("span", { className: "dshCodexCopyFeedbackText" }, t("copied")) : null,
                  ),
                  React.createElement(
                    "span",
                    { className: "dshCodexSrOnly", role: "status", "aria-live": "polite" },
                    copied ? t("copied") : "",
                  ),
                ),

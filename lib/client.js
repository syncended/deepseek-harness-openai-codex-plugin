...
`;
    ...
    const dictionaries = {
      zh: {
        ...
        "limits.primary": "5 小时限额", "limits.secondary": "7 天限额", "limits.named": "{name} · {window}", "limits.codeReview": "代码审查限额",
      ...
      en: {
        ...
        "limits.primary": "5-hour limit", "limits.secondary": "7-day limit", "limits.named": "{name} · {window}", "limits.codeReview": "Code review limit",
    ...
    function usageRows(value, t) {
      ...
      const additional = (value.additional ?? []).map((item, index) => {
      ...
      return [
        ...base.map((row) => ({ ...row, label: windowLabel(row.value, row.fallback, t) })),
        ...additional,
      ].sort((left, right) => (left.value.windowSeconds ?? Infinity) - (right.value.windowSeconds ?? Infinity));
    }
        ...
        open
          ? React.createElement(
              ...
              React.createElement(
                ...
                rows.map((row) => React.createElement(LimitRow, { key: row.key, label: row.label, value: row.value, t })),
